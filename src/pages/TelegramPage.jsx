...
import {useGetChartsDayTwitterQuery , useGetEhsasTelegramNewsHeatMapQuery,
     useGetEhsasTelegramNewsParallelQuery, useGetEhsasTelegramNewsPerMonthQuery,
      useGetEhsasTelegramNewsPerYearQuery, useGetTelegramBarChartQuery,
       useGetTelegramBubbleChartQuery, useGetTelegramEhsasQuery,
        useGetTelegramHeatMapQuery, useGetTelegramPieChartQuery,
          useGetTelegramRadarDOQuery,
          useGetTelegramRadarYekQuery,
          useGetTelegramStackedColumnsQuery, useGetTelegramSwarmMapQuery,
...
function TelegramPage() {
    ...
    const {data :dataBubble , isLoading : loadingBubble , isSuccess : successBubble} = useGetTelegramBubbleChartQuery('Online' , {pollingInterval : 50000})
    const {data : dataBar , isLoading : loadingBar , isSuccess : successBar} = useGetTelegramBarChartQuery('Online' , {pollingInterval : 50000})
    const {data : dataPie , isLoading : loadingPie , isSuccess : successPie} = useGetTelegramPieChartQuery('Online' , {pollingInterval : 50000})
    const {data : dataSwarm, isLoading : loadingSwarm , isSuccess:successSwarm} = useGetTelegramSwarmMapQuery('Online' , {pollingInterval : 50000})
    const {data : dataStacked , isLoading : loadingStacked , isSuccess : successStacked} = useGetTelegramStackedColumnsQuery('Online' , {pollingInterval : 50000})
    const {data : dataHeat , isLoading:loadingHeat , isSuccess : successHeat} = useGetTelegramHeatMapQuery('Online' , {pollingInterval : 50000})
    const {data : dataEhsas , isLoading:loadingEhsas , isSuccess: successEhsas} = useGetTelegramEhsasQuery('Online' , {pollingInterval : 50000})
    const {data : dataRadarYek , isloading: loadingRadarYek , isSuccess : successRadarYek} = useGetTelegramRadarYekQuery('Online' , {pollingInterval : 50000})
    const {data : dataRadarDo , isLoading : loadingRadarDo , isSuccess : successRadarDo} = useGetTelegramRadarDOQuery('Online' , {pollingInterval : 50000})
  ...
  options={{
    ...
    />
       ...
       <Row className='m-3'>
        ...
        <Col xs={24} sm={24} md={12} lg={12} xl={12} xxl={12}>
        <Divider   orientation='left'  style={{fontSize: 15 }} className='px-5'>
        کدام رسانه‌های داخلی و خارجی بیشتر به انتخابات پرداخته‌اند؟
