...
import { useGetInstagramBarChartQuery, useGetInstagramBubbleChartQuery, useGetInstagramHeatChartQuery, useGetInstagramPieChartQuery, useGetInstagramStackedChartQuery, useGetInstagramSwarmChartQuery, useGetTwitterBarChartQuery, useGetTwitterBubbleChartQuery, useGetTwitterEhsasQuery, useGetTwitterHeatChartQuery, useGetTwitterPieChartQuery, useGetTwitterRadarDoChartQuery, useGetTwitterRadarYekChartQuery, useGetTwitterStacketChartQuery, useGetTwitterSwarmChartQuery } from '../query/Charts'
...
function TwitterPage() {
    ...
    const {data :dataBubble , isLoading : loadingBubble , isSuccess : successBubble} = useGetTwitterBubbleChartQuery('Online' , {pollingInterval : 50000})
    const {data : dataBar , isLoading : loadingBar , isSuccess : successBar} = useGetTwitterBarChartQuery('Online' , {pollingInterval : 50000})
    const {data : dataPie , isLoading : loadingPie , isSuccess : successPie} = useGetTwitterPieChartQuery('Online' , {pollingInterval : 50000})
    const {data : dataSwarm, isLoading : loadingSwarm , isSuccess:successSwarm} = useGetTwitterSwarmChartQuery('Online' , {pollingInterval : 50000})
    const {data : dataStacked , isLoading : loadingStacked , isSuccess : successStacked} = useGetTwitterStacketChartQuery('Online' , {pollingInterval : 50000})
    const {data : dataHeat , isLoading:loadingHeat , isSuccess : successHeat} = useGetTwitterHeatChartQuery('Online' , {pollingInterval : 50000})
    const {data : dataEhsas , isLoading:loadingEhsas , isSuccess: successEhsas} = useGetTwitterEhsasQuery('Online' , {pollingInterval : 50000})
    const {data : dataRadarYek , isloading: loadingRadarYek , isSuccess : successRadarYek} = useGetTwitterRadarYekChartQuery('Online' , {pollingInterval : 50000})
    const {data : dataRadarDo , isLoading : loadingRadarDo , isSuccess : successRadarDo} = useGetTwitterRadarDoChartQuery('Online' , {pollingInterval : 50000})
    ...
    return( 
        <>
        <Divider    style={{fontSize: 15 }} className='px-5'>
        روند تولید و مصرف موضوعات مهم کشور در 24 ساعت اخیر چگونه بوده است؟
        </Divider> 
        ...
        <Col xs={24} sm={24} md={12} lg={12} xl={12}  >
            ...
            <div className='h-96  border border-black  rounded-lg  m-2 p-6 '>
               ...
               {!loadingHeat && successHeat ? (
                 <HeatCharts data={dataHeat} min={-1000} max={3000}/>
               ) : (
                <Skeleton variant="rectangular"  height={150}  className='p-2'    />
               )}
